...
def tsne_visualization(newsletters, color_by=None):
    ...
    color_arg = None
    if hasattr(tsne_visualization, 'color_override') and tsne_visualization.color_override is not None:
    ...
    fig = px.scatter(
        ...
        hover_name="title",
        color=color_arg,
        title=f"t-SNE Clustering of Newsletters" + (f" (colored by tag)" if hasattr(tsne_visualization, 'color_override') and tsne_visualization.color_override is not None else (f" (colored by '{color_by}')" if color_by else "")),
        width=600,
        height=600
    )
